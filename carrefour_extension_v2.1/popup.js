...
importBtn.addEventListener('click', function() {
  ...
  // קבלת מצב היבוא שנבחר (החלפה או הוספה)
  ...
  chrome.tabs.query({active: true, currentWindow: true}, function(tabs) {
    ...
    if (activeTab.url.includes('carrefour.co.il')) {
      ...
      chrome.tabs.sendMessage(activeTab.id, {
        action: "importCart",
        data: csvData,
        mode: importMode // הוספת פרמטר חדש - מצב היבוא
      }, function(response) {
        ...
        if (chrome.runtime.lastError) {
          ...
          if (confirm('התוסף לא הצליח לתקשר עם הדף. האם לרענן את הדף ולנסות שוב?')) {
            chrome.storage.local.set({ 
              'carrefour_csv_import_data': csvData,
              'carrefour_csv_import_mode': importMode // שמירת מצב היבוא
            }, function() {
              chrome.tabs.reload(activeTab.id);
            });
          }
